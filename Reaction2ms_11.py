...
x_time_2_before = df['Reaction time_2_Submitted Before (ms)'].dropna()
y_time_2_after = df['Reaction time_2_Submitted After (ms)'].dropna()
...
after_mean = round(np.mean(y_time_2_after),6)
before_mean = round(np.mean(x_time_2_before),6)

# Degrees of freedom for the two samples
df1 = len(x_time_2_before) - 1
df2 = len(y_time_2_after) - 1
...
statistic, p_value = levene(x_time_2_before , y_time_2_after)

variance1 = round(np.var(x_time_2_before, ddof=1),6)  # ddof=1 for sample variance
variance2 = round(np.var(y_time_2_after, ddof=1),6)
#the F-statistic
...
#Box plotting 
out=plt.boxplot([x_time_2_before, y_time_2_after],meanline=True,showfliers=True, flierprops=dict(markerfacecolor='pink', marker='o'),patch_artist=True)
